...
export const contactFormSchema = z.object({
  name: z.string()
    ...
    .pipe(z.string().min(1, 'お名前は必須です'))
    .pipe(z.string().max(100, '名前は100文字以内で入力してください')),
  
  email: z.string()
    .transform(str => str.trim())
    .pipe(z.string().min(1, 'メールアドレスは必須です'))
    .pipe(z.string().email('有効なメールアドレスを入力してください'))
    .pipe(z.string().max(255, 'メールアドレスは255文字以内で入力してください')),
  
  subject: z.string()
    .transform(str => str.trim())
    .pipe(z.string().min(1, '件名は必須です'))
    .pipe(z.string().max(200, '件名は200文字以内で入力してください')),
  
  message: z.string()
    .transform(str => str.trim())
    .pipe(z.string().min(1, 'お問い合わせ内容は必須です'))
    .pipe(z.string().min(10, 'お問い合わせ内容は10文字以上で入力してください'))
    .pipe(z.string().max(2000, 'お問い合わせ内容は2000文字以内で入力してください')),
